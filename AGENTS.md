...
- **Clicking a hidden barangay**: triggers `drillUp(2)` (back to municipality)
...
### Data cross-walk pitfalls

Property schemas differ between GeoJSON levels — `_filterToParent` and `_featureName` handle these:

| Level | Key props in GeoJSON | Filter parent match |
|-------|---------------------|-------------------|
| 0 (Region) | `Region`, `PSGC` | — |
| 1 (Province) | `PROVINCE`, `PSGC_P` | — |
| 2 (Municipality) | `Province`, `Municipali`, `PSGC` | `Province` ↔ parent `PROVINCE` |
| 3 (Barangay) | `NAME_1`, `NAME_2`, `NAME_3` | `NAME_2` ↔ parent `Municipali` (uses `.includes()` — handles "Baguio" vs "Baguio City" mismatch) |

All comparisons use `.toUpperCase()` with `.includes()` (not `===`) to tolerate data mismatches between NAMRIA municipal and barangay datasets.
