 ...
 *
 * Usage: node scripts/preprocess_watersheds.js
 */
...
function main() {
  ...
  const namriaProvRaw = fs.readFileSync(path.join(geoDir, 'CAR NAMRIA Provincial Boundary.geojson'), 'utf8');
  const namriaProvData = JSON.parse(namriaProvRaw);

  // Load NAMRIA Municipalities
  const namriaMuniRaw = fs.readFileSync(path.join(geoDir, 'CAR NAMRIA Municipal Boundary.geojson'), 'utf8');
  const namriaMuniData = JSON.parse(namriaMuniRaw);

  // Load CAD Municipalities (CAD Provincial is identical per AGENTS.md)
  const cadMuniRaw = fs.readFileSync(path.join(geoDir, 'CAR CAD Municipal Boundary.geojson'), 'utf8');
  const cadMuniData = JSON.parse(cadMuniRaw);

  // Load Watersheds
  const wsRaw = fs.readFileSync(path.join(geoDir, 'CAR Watersheds.geojson'), 'utf8');
  ...
  const outPath = path.join(geoDir, 'watershed-intersections.json');
